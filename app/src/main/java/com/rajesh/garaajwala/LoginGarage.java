...
public class LoginGarage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_login_garage);
        ...
        b_login.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                MyHandler db = new MyHandler(LoginGarage.this);
                String login_email = email.getEditText().getText().toString();
                String login_password = password.getEditText().getText().toString();
                boolean check = db.loginGaraaj(login_email, login_password);
                if (check){
//                    List<Garaaj> users = db.get(login_email);
//                    if(users != null) {
...
//                    Log.d("goon","" + details.get("fullnameS"));
                    startActivity(new Intent(getApplicationContext(),GarageDashboard.class));
                    finish();
